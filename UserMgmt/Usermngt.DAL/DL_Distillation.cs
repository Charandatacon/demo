        ...
        public static List<Distillation> GetList()
        {
            ...
            using (NpgsqlConnection cn = new NpgsqlConnection(ConfigurationManager.ConnectionStrings["CASEMGMT"].ToString()))
            {
                ...
                try
                {
                    ...
                    cmd = new NpgsqlCommand("select d.*,b.vat_name,c.party_name,c.party_code,a.setup_date,a.total_qty_transferred,a.setup_complete,d.financial_year as ff from exciseautomation.rawmaterial_fermenter a inner join exciseautomation.vat_master b on a.tofermentervat=b.vat_code inner join exciseautomation.party_master c on a.party_code=c.party_code inner join exciseautomation.distillation d on a.rawmaterial_fermenter_id=d.rawmaterial_fermenter_id  order by d.distillation_date desc", cn);
                    ...
                    if (dr.HasRows)
                    {
                        ...
                        while (dr.Read())
                        {
                            ...
                            record.financial_year = dr["ff"].ToString();
        ...
        public static string Insert(Distillation Distillation)
        {
            ...
            using (NpgsqlConnection cn = new NpgsqlConnection(ConfigurationManager.ConnectionStrings["CASEMGMT"].ToString()))
            {
               ...
               // where financial_year = '"+Distillation.financial_year+"'
                ...
                NpgsqlCommand cmd3 = new NpgsqlCommand("select rawmaterial_fermenter_id from exciseautomation.rawmaterial_fermenter where setup_date='" + Distillation.setup_date + "' and tofermentervat='" + Distillation.vat_code + "' and user_id='"+Distillation.user_id+ "' and  setup_complete ='N' and financial_year='" + Distillation.getfinancial_year + "' ", cn);
                int c = Convert.ToInt32(cmd3.ExecuteScalar());

                NpgsqlCommand cmd4 = new NpgsqlCommand("select fermenter_setup_id from exciseautomation.fermenter_setup where rawmaterial_fermenter_id='" + c + "' and financial_year='" + Distillation.getfinancial_year + "' ", cn);
