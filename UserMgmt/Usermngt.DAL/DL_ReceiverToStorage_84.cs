        ...
        public static List<FReceiverOuput> GetListreceiverop(string party_code)
        {
            ...
            using (NpgsqlConnection cn = new NpgsqlConnection(ConfigurationManager.ConnectionStrings["CASEMGMT"].ToString()))
            {
                ...
                try
                {
                    NpgsqlCommand cmd;
                    if (party_code == null || party_code == "All")
                        cmd = new NpgsqlCommand("SELECT distinct(a.removal_date) as removal_date,b.financial_year FROM exciseautomation.fermenter_receiver_output a inner join  exciseautomation.fermenter_receiver b on a.fermenter_receiver_id=b.fermenter_receiver_id where a.moved_to_nextstage='N' and b.record_status='A'  order by a.removal_date ", cn);
                    else
                        cmd = new NpgsqlCommand("SELECT distinct(a.removal_date) as removal_date,b.financial_year FROM exciseautomation.fermenter_receiver_output a inner join  exciseautomation.fermenter_receiver b on a.fermenter_receiver_id=b.fermenter_receiver_id where a.moved_to_nextstage='N' and b.record_status='A' and b.party_code='" + party_code+"' order by a.removal_date ", cn);
