   ...
   public class DL_RequestForTransportPass
    {
        ...
        public static ReaquestForPass GetRequest(string requestid, string pass_type, string party_code, string party_type, string financial_year)
        {
            ...
            using (NpgsqlConnection cn = new NpgsqlConnection(ConfigurationManager.ConnectionStrings["CASEMGMT"].ToString()))
            {
                ...
                try
                {
                    ...
                    {
                        pass_type = "EXP";
                        cmd = new NpgsqlCommand("select a.*,b.lifted_qty,b.financial_year as rfinancial_year,b.req_qty as req_qty1,b.record_status as record_status1,c.tenderno,c.party_code,c.req_nocno ,c.issue_nocno,c.issue_date,h.product_name,h.product_code,b.request_for_pass_id,c.valid_upto,b.approved_qty,d.customer_id,d.cust_name,d.cust_address,c.issue_nocno,g.noc,g.pass,f.product_code,f.product_name,b.digital_lock_no,b.vehicle_no from exciseautomation.noc_depotdetail a inner join exciseautomation.request_for_pass b on a.noc_id=b.rrnoc_request_id and a.noc_depotdetail_id=b.noc_depotdetail_id  left join exciseautomation.noc c on a.noc_id = c.noc_id  and b.party_code=c.party_code left join exciseautomation.customer_master d on c.customer_id = d.customer_id inner join exciseautomation.product_master f on f.product_code=c.noc_for left join exciseautomation.document_format_master g on g.party_code=c.party_code inner join exciseautomation.product_master h on h.product_code=f.product_code  where b.request_for_pass_id='" + requestid + "' and b.pass_type='" + pass_type + "' and b.party_code='" + party_code + "' and b.financial_year='"+financial_year+"'", cn);
        ...
        public static List<ReaquestForPass> GetNOCList(string user_id ,string party_type)
        {
            ...
            using (NpgsqlConnection cn = new NpgsqlConnection(ConfigurationManager.ConnectionStrings["CASEMGMT"].ToString()))
            {
                ...
                try
                {
                    NpgsqlCommand cmd = new NpgsqlCommand("select a.*,b.valid_upto,c.product_name,b.customer_id,b.noc_total_qty,b.party_code as party,d.noc,b.issue_nocno,b1.cust_name from exciseautomation.request_for_pass a inner join exciseautomation.noc b on a.rrnoc_request_id=b.noc_id  and a.party_code =b.party_code inner join exciseautomation.product_master as c on b.noc_for=c.product_code left join exciseautomation.document_format_master d on a.party_code=d.party_code left join exciseautomation.customer_master b1 on b.customer_id=b1.customer_id where a.pass_type='" + party_type+ "' and a.record_active='true'  order by b.valid_upto,a.request_for_pass_id desc", cn);
        ...
        public static List<ReaquestForPass> Searchena(string tablename, string column, string value, string passtype)
        {
            ...
            using (NpgsqlConnection cn = new NpgsqlConnection(ConfigurationManager.ConnectionStrings["CASEMGMT"].ToString()))
            {
                ...
                {
                    NpgsqlCommand cmd1;
                    using (NpgsqlCommand cmd = new NpgsqlCommand("select a.*,b.valid_upto,c.product_name,b.customer_id,b.noc_total_qty,b.party_code as party,d.noc,b.issue_nocno,b1.cust_name from exciseautomation.request_for_pass a inner join exciseautomation.noc b on a.rrnoc_request_id=b.noc_id and a.party_code =b.party_code inner join exciseautomation.product_master as c on b.noc_for=c.product_code left join exciseautomation.document_format_master d on a.party_code=d.party_code left join exciseautomation.customer_master b1 on b.customer_id=b1.customer_id where a.pass_type='" + passtype + "' and " + column + " Ilike '%" + value + "%' and a.record_active='true'  order by rrnoc_request_id,request_for_pass_id", cn))
                    {
            ...
            using (NpgsqlConnection cn = new NpgsqlConnection(ConfigurationManager.ConnectionStrings["CASEMGMT"].ToString()))
            {
                ...
                try
                {
                    ...
                    if (pass_type == "E")
                    {
                        ...
                        cmd = new NpgsqlCommand("select a.*,b.valid_upto as valid_date,b.noc_total_qty as allocation_qty,b.customer_id as molasses_supplier,b.party_code,b.nocdate as rr_date,b.noc_total_qty as rr_approved_qty,c.Product_name,d.cust_name from exciseautomation.request_for_pass a inner join exciseautomation.noc b on a.rrnoc_request_id = b.noc_id and a.party_code  =b.party_code inner join exciseautomation.product_master as c on b.noc_for = c.product_code inner join exciseautomation.Customer_master as d on b.customer_id = d.customer_id  where a.request_for_pass_id='" + requestid + "' and pass_type='" + pass_type + "' and a.financial_year='"+financial_year+"'   order by a.request_for_pass_id", cn);
                    }
                    else
                    {
                        pass_type = "DOM";
            ...
            using (NpgsqlConnection cn = new NpgsqlConnection(ConfigurationManager.ConnectionStrings["CASEMGMT"].ToString()))
            {
                ...
                try
                {
                    ...
                    else
                    {
                        ...
                        NpgsqlCommand cmd = new NpgsqlCommand("Select case when sum(req_qty) is null then 0 else sum(req_qty) end approved_qty from exciseautomation.request_for_pass  where rrnoc_request_id='" + id + "' and pass_type='" + pass_type + "' and record_status!='R' and record_status !='N'", cn);
            ...
            using (NpgsqlConnection cn = new NpgsqlConnection(ConfigurationManager.ConnectionStrings["CASEMGMT"].ToString()))
            {
                ...
                try
                {
                    ...
                    foreach (DataRow dr in dt.Rows)
                    {
                        ...
                        cmd = new NpgsqlCommand("select case when sum(req_qty) is null then 0 else sum(req_qty) end as req_qty from exciseautomation.request_for_pass where record_status!='R' and record_status!='N' and rrnoc_request_id='" + no.noc_id + "' and party_code='" + no.party_code + "'", cn);
