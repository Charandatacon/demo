   ...
   public class DL_Form82
    {
        ...
        public static Form82 Getopeningbalance(string Vat_code, string party_Code)
        {
            Form82 vat = new Form82();
            using (NpgsqlConnection cn = new NpgsqlConnection(ConfigurationManager.ConnectionStrings["CASEMGMT"].ToString()))
            {
                ...
                try
                {
                    //  NpgsqlCommand cmd = new NpgsqlCommand("select a.vat_availablecapacity from exciseautomation.vat_master a where vat_code='" + Vat_code + "' and party_code='" + party_Code + "'", cn);
                    // NpgsqlCommand cmd = new NpgsqlCommand("select   coalesce( SUM(totalpurchase) - SUM(totalconsumption + total_wastage), 0) as closingbalance  from  exciseautomation.distillery_mollasses_stock dms  where vat_code ='" + Vat_code + "' and financial_year='"+party_Code+"'", cn);
                    NpgsqlCommand cmd = new NpgsqlCommand("select  openingbalancevalue  from exciseautomation.openingbalance  where vat_code ='" + Vat_code + "' and financial_year='" + party_Code + "' ", cn);
                    cmd.CommandType = System.Data.CommandType.Text;
                    NpgsqlDataReader dr = cmd.ExecuteReader();
                    if (dr.HasRows)
                    {
                        ...
                        {
                            // vat.vat_availablecapacity = Convert.ToDouble(dr["vat_availablecapacity"].ToString());
                            vat.vat_availablecapacity = Convert.ToDouble(dr["openingbalancevalue"].ToString());
                        }
                    }
                }
                catch (Exception ex)
        ...
        public static List<Form82> Getdistinctdate(string party)
        {
            ...
            using (NpgsqlConnection cn = new NpgsqlConnection(ConfigurationManager.ConnectionStrings["CASEMGMT"].ToString()))
            {
                ...
                try
                {
                    using (NpgsqlCommand cmd = new NpgsqlCommand("select distinct(a.setup_date),a.financial_year from exciseautomation.rawmaterial_fermenter a inner join exciseautomation.vat_master b on a.tofermentervat=b.vat_code inner join exciseautomation.party_master c on a.party_code=c.party_code where a.setup_complete='N' and a.record_status='Y' and c.party_code='" + party+"' order by a.setup_date", cn))
                    {
                        cmd.CommandType = System.Data.CommandType.Text;
                        //cmd.Parameters.AddWithValue("@UserID", userid);
                        NpgsqlDataReader dr = cmd.ExecuteReader();
