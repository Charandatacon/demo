   ...
   public class DL_Party_Master
    {
        ...
        public static List<Party_Master> GetList()
        {
            ...
            using (NpgsqlConnection cn = new NpgsqlConnection(ConfigurationManager.ConnectionStrings["CASEMGMT"].ToString()))
            {
                ...
                try
                {
                    ...
                    using (NpgsqlCommand cmd = new NpgsqlCommand("select distinct a.*,b.party_type_name from exciseautomation.party_master a inner join exciseautomation.party_type_master b on a.party_type_code=b.party_type_code left join exciseautomation.party_financial_yr c on b.party_type_code=c.party_type_code  order by a.party_name", cn))
                    {
                        cmd.CommandType = System.Data.CommandType.Text;
                        //cmd.Parameters.AddWithValue("@UserID", userid);
                        NpgsqlDataReader dr = cmd.ExecuteReader();
        ...
        public static string changefinancialyear(string tablename, string column, string value,string financialendate, string startdates, string financial_year)
        {
            string value1 = "";
            string value2 = "";
            ...
            using (NpgsqlConnection cn = new NpgsqlConnection(ConfigurationManager.ConnectionStrings["CASEMGMT"].ToString()))
            {
                ...
                {
                    NpgsqlCommand cmd;
                    NpgsqlCommand cmd9 = new NpgsqlCommand("select financial_year  from exciseautomation.party_master where party_code='"+column+"' ", cn);
                  string party_year = cmd9.ExecuteScalar().ToString();
                    OpeningBalance request = new OpeningBalance();
                        request.vats = DL_Party_Master.GetOpeningBalanceList(column,party_year);
                    ...
                    var year = from s in Org_Finacial
                               ...
                               select s;
                    NpgsqlCommand cmd3 = new NpgsqlCommand("update exciseautomation.party_master set financial_year='" + financial_year + "', lastmodified_date='" + DateTime.Now.ToString("dd-MM-yyyy") + "',enddate='" + financialendate + "',startdate='" + startdates + "',approver_level='Y' where party_code = '" + column + "'", cn);
                    // for suggarmill  NpgsqlCommand cmd3 = new NpgsqlCommand("update exciseautomation.party_master set financial_year='"+year.ToList()[0].financial_year+ "', lastmodified_date='" + DateTime.Now.ToString("dd-MM-yyyy") + "',enddate='"+financialendate+ "',startdate='"+startdates+ "',approver_level='Y' where party_code = '" + column + "'", cn);
                    ...
                    // for suggarmill if(year.ToList()[0].financial_year != request.vats[0].financial_year)
                    if (financial_year != request.vats[0].financial_year)
                    {
                        ...
                        if (tablename == "openingbalance")
                        {
                            ...
                            for (int i = 0; i < request.vats.Count; i++)
                            {
                                ...
                                if (financial_year != request.vats[0].financial_year)
                                {
                                    ...
                                    {
                                        NpgsqlCommand cmd1 = new NpgsqlCommand("INSERT INTO exciseautomation.openingbalance(openingbalance_id, vat_code, uom_code, storagecontent, openingbalancevalue,openingbalanceyear,creation_date, user_id,record_status,remarks,party_code,financial_year)VALUES('" + max + "','" + request.vats[i].vat_code + "', '" + request.vats[i].uom_code + "', '" + request.vats[i].storage_content + "',0,'" + year.ToList()[0].financial_year.ToString() + "', '" + DateTime.Now.ToShortDateString() + "', '" + value + "','N','" + request.vats[i].remarks + "','" + request.vats[i].party_code + "','" + financial_year + "')", cn);
                                        int n = Convert.ToInt32(cmd1.ExecuteNonQuery());
                                        max++;
                                        if (n == 0)
                                        {
