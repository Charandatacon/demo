...
namespace UserMgmt
{
    public partial class ChangeFinancialYearList : System.Web.UI.Page
    {
        ...
        protected void Page_Load(object sender, EventArgs e)
        {
            if (!IsPostBack)
            {
                ...
                var year = from s in Org_Finacial
                           ...
                           select s;
                if (user.role_name.Trim() == "Applicant")
                {
                    ...
                    for (int i = 0; i < grdpartymasterList.Rows.Count; i++)
                    {
                        string financial_year = (grdpartymasterList.Rows[i].FindControl("lblfinancial") as Label).Text; 
                        string enddate = (grdpartymasterList.Rows[i].FindControl("lblupdateddate") as Label).Text;
                        if (year.ToList()[0].financial_year == financial_year && DateTime.Now.Date < Convert.ToDateTime(enddate))
                            (grdpartymasterList.Rows[i].FindControl("btnEdit") as LinkButton).Visible = false;
                        else
                            (grdpartymasterList.Rows[i].FindControl("btnEdit") as LinkButton).Visible = true;
                        if(DateTime.Now.Date < Convert.ToDateTime(enddate)&& partymasters[i].record_status!="Y")
                        {
                            (grdpartymasterList.Rows[i].FindControl("btnEdit") as LinkButton).Visible = false;
                        }
               ...
               if (user.role_name.Trim() == "Bond Officer")
                {
                    var partynames = from s in partymasters
                                     where s.party_code == user.party_code && (s.record_status == "A" || s.record_status == "Y")
                                     select s;
                    grdpartymasterList.DataSource = partynames.ToList();
                    grdpartymasterList.DataBind();
                    for (int i = 0; i < grdpartymasterList.Rows.Count; i++)
                ...
                {
                    var partynames = from s in partymasters
                                     where s.party_code != "ALL" &&(s.record_status == "C" || s.record_status == "A"|| s.record_status == "D")/* && s.record_status != "Y"*/
                                     select s;
                    grdpartymasterList.DataSource = partynames.ToList();
                    grdpartymasterList.DataBind();
                    for (int i = 0; i < grdpartymasterList.Rows.Count; i++)
                ...
                {
                    var partynames = from s in partymasters
                                     where s.party_code != "ALL" && (s.record_status == "C" || s.record_status == "D") /*&&  s.record_status!="A" && s.record_status != "Y"*/
                                     select s;
                    grdpartymasterList.DataSource = partynames.ToList();
                    grdpartymasterList.DataBind();
                    for (int i = 0; i < grdpartymasterList.Rows.Count; i++)
            ...
            if (Session["pmsearch"] != null && Session["pmtext"] != null)
            {
                ...
                if (ddsearch.SelectedValue != "Select")
                {
                    ...
                    if (txtpage != null)
                    {
                        ...
                        {
                            string financial_year = (grdpartymasterList.Rows[i].FindControl("lblfinancial") as Label).Text;
                            if (year.ToList()[0].financial_year == financial_year)
                                (grdpartymasterList.Rows[i].FindControl("btnEdit") as LinkButton).Visible = false;
                            else
                                (grdpartymasterList.Rows[i].FindControl("btnEdit") as LinkButton).Visible = true;
                        }
                        ...
                        {
                            var partynames = from s in partymasters
                                             where s.party_code!= "ALL" && s.record_status == "A" && (s.record_status == "C" || s.record_status == "A" || s.record_status == "D")
                                             select s;
                            grdpartymasterList.DataSource = partynames.ToList();
                            grdpartymasterList.DataBind();
                            for (int i = 0; i < grdpartymasterList.Rows.Count; i++)
                        ...
                        {
                            var partynames = from s in partymasters
                                             where s.party_code != "ALL" && (s.record_status == "C"  || s.record_status == "D")
                                             select s;
                            grdpartymasterList.DataSource = partynames.ToList();
                            grdpartymasterList.DataBind();
                            for (int i = 0; i < grdpartymasterList.Rows.Count; i++)
        ...
        protected void AddRecord_Click(object sender, EventArgs e)
        {
            ...
            Session["rtype"] = "0";
            Response.Redirect("ChangeFinancialYear.aspx");
        }
        ...
        protected void btnView_Click(object sender, EventArgs e)
        {
            ...
            string financial_year = (gvr.Cells[gvr.Cells.Count - 1].FindControl("lblfinancial") as Label).Text;
            Session["CHYfinancial_year"] =financial_year;
            Session["Party_Code1"] = Party_Code;
            Session["UserID"] = Session["UserID"];
            Session["rtype"] = "1";
            Response.Redirect("ChangeFinancialYear.aspx");
        }
        ...
        protected void btnsearch_Click(object sender, EventArgs e)
        {
            ...
            if (ddsearch.SelectedValue != "Select")
            {
                ...
                if (txtpage != null)
                {
                    ...
                    {
                        var partynames = from s in partymasters
                                         where s.party_code != "ALL" && (s.record_status == "C" || s.record_status == "A" || s.record_status == "D")
                                         select s;
                        grdpartymasterList.DataSource = partynames.ToList();
                        grdpartymasterList.DataBind();
                        for (int i = 0; i < grdpartymasterList.Rows.Count; i++)
        ...
        protected void txtpage_TextChanged(object sender, EventArgs e)
        {
            ...
            //{
            //    var partynames = from s in partymasters
            //                     where s.record_status == "A" && s.record_status == "Y"
            //                     select s;
            //    grdpartymasterList.DataSource = partynames.ToList();
            //    grdpartymasterList.DataBind();
            //    for (int i = 0; i < grdpartymasterList.Rows.Count; i++)
