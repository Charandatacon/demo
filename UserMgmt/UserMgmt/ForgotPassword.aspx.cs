    ...
    public partial class ForgotPassword : System.Web.UI.Page
    {
       ...
       // string district = "";
        ...
        protected void btnSubmit_Click(object sender, EventArgs e)
        {
            ...
            var otp_check_cmd = new NpgsqlCommand("SELECT user_id,linkexpirationtime as OtpTrue from exciseautomation.user_registration where emailotp='" + txtEmailOtp.Value + "' and smsotp='" + txtSMSOtp.Value + "'", conn);
            NpgsqlDataReader otp_pgreader = otp_check_cmd.ExecuteReader();
            DataTable dt_otp = new DataTable();
            dt_otp.Load(otp_pgreader);


            var command_expire = new NpgsqlCommand("SELECT (NOW()::timestamp) as CurrentTime;", conn);
            NpgsqlDataReader pgreader_expire = command_expire.ExecuteReader();

            DataTable dt_expire = new DataTable();
            dt_expire.Load(pgreader_expire);

            DataSet ds_expire = new DataSet();
            ds_expire.Tables.Add(dt_expire);

            DateTime time = DateTime.Now;
            DateTime ctime = DateTime.Now;

            if (dt_otp.Rows.Count > 0) { 
            time = Convert.ToDateTime(dt_otp.Rows[0]["OtpTrue"].ToString());
            ctime = Convert.ToDateTime(dt_expire.Rows[0]["CurrentTime"].ToString()); }

            string d = Convert.ToDateTime(dt_expire.Rows[0]["CurrentTime"]).ToString("yyyy-MM-dd hh:mm:ss");
            ctime = Convert.ToDateTime(d);

           
            if (dt_otp.Rows.Count > 0 && ctime<time)
            {


                if (txtConfirmPassword.Value != txtNewPassword.Value)
                {
                    string script = "alert(\"New Password & Confirm New Password mismatch!!!!!\");";
                    ScriptManager.RegisterStartupScript(this, GetType(),
                                          "ServerControlScript", script, true);
                }

                else if(txtNewPassword.Value.Length<=9 || txtConfirmPassword.Value.Length <= 9)
                {
                    string script = "alert(\"New Password & Confirm New Password length must greater than 9 characters!!!!!\");";
                    ScriptManager.RegisterStartupScript(this, GetType(),
                                          "ServerControlScript", script, true);
                }
                ...
                {
                    string qrystrg = Request.QueryString["um"];
                    ...
                    string pwd = encryption(txtNewPassword.Value);

                    var command_update_expirationtime = new NpgsqlCommand("update exciseautomation.user_registration set password='" + pwd + "',emailotp='',smsotp='' where user_id='" + txtUserId.Value + "'", conn);
                    command_update_expirationtime.ExecuteScalar();

                    conn.Close();
                    string script = "alert(\"Password reset successfully!!!\");";
                    ScriptManager.RegisterStartupScript(this, GetType(),
                                          "ServerControlScript", script, true);
            ...
            {
                string script = "alert(\"Wrong OTP or OTP Expired!!!!!\");";
                ScriptManager.RegisterStartupScript(this, GetType(),
                                      "ServerControlScript", script, true);
            }
